<?php
  class login_class{
    ...
    public function REGISTER($username, $firstname, $surname, $email, $password, $pri_ip, $department){
      ...
      if (($username || $firstname || $surname || $email || $password || $department) == "") {
        ...
        $query = $this->db->prepare("INSERT INTO users(username, firstname, surname, email, password, signup, last_login, pri_ip, pri_os, pri_browser, department) VALUES(:username, :firstname, :surname, :email, :password, now(), now(), :ip, :os, :browser, :department)");
        $query->execute(array(":username" => $username, ":firstname" => $firstname, ":surname" => $surname, ":email" => $email, ":password" => $password_h, ":ip" => $pri_ip, ":os" => $os, ":browser" => $browser, ":department" => $department));
        ...
        $mail->FromName = "Team Students Connect";
        $mail->addAddress($email);               // Name is optional
        $mail->addReplyTo($this->gmail, 'Team Students Connect');
        ...
        $mail->Subject = 'Verify your Students Connect account';

        $mail->Body = "<span>Hello, You received this message because you created an account on Students Conenct.<span><br>
        <span>Click on button below to verify your Students Connect account and explore Notes.</span><br><br>
